...
const createCourse = async (req, res,next) => {
  ...
  try {
    ...
    const assg_result = await cloudinary.uploader.upload(AssTask.tempFilePath, { folder: "Assignment Question" },)
    ...
    const quiz_result = await cloudinary.uploader.upload(quiz.tempFilePath, { folder: "Quiz Paper" },)
    ...
    const mid_result = await cloudinary.uploader.upload(mid.tempFilePath, { folder: "Mid Paper" },)
    ...
    const final_result = await cloudinary.uploader.upload(final.tempFilePath, { folder: "Final Paper" },)
